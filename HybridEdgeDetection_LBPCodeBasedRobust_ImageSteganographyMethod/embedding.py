...
def embed(cover_path: str, secret_text: str, n_bits: int):
    ...
    smoothed = strip_lsb_image(cover, n_bits)
    ...
    stego  = cover.copy()
    tc     = np.zeros_like(cover, dtype=np.uint8)
...
def embed_all_images(cover_dir: str, stego_dir: str, secret_text: str, n_bits: int):
    ...
    for filename in filenames:
        ...
        results.append((display_name, stego))
    ...
    for i, (name, stego_img) in enumerate(results):
        ...
        plt.imshow(stego_img, cmap='gray', vmin=0, vmax=255)
        plt.axis('off')
...
if __name__ == "__main__":
    ...
    print("Running batch embedding (Figure 5) ...")
