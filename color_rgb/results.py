...
def _load_cover_images_rgb(cover_dir: str):
...
def show_metrics_table(cover_dir: str = COVER_DIR, secret_text: str = LONG_MESSAGE, n_bits: int = N_BITS):
    images = _load_cover_images_rgb(cover_dir)
    if not images:
        print("[!] No images found.")
        return

    print("Computing metrics RGB ... please wait.\n")
    rows = []
...
def show_ttest_table(cover_dir: str = COVER_DIR, secret_text: str = LONG_MESSAGE, n_bits: int = N_BITS):
    ...
    print("Computing t-test values RGB ... please wait.\n")
    rows = []

    for filename, display_name, cover, path in images:
        print(f"  Processing {display_name} ...")
        stego, _, _ = embed_rgb(path, secret_text, n_bits)
        rows.append([display_name, f"{m.t_test(cover, stego):.4f}"])

    _render_table(rows, ["Image Name", "t-Test Value"], "Table 4. t-Test values (RGB)")
