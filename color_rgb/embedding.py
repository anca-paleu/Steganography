...
def _embed_channel(cover_channel, message_bits, n_bits: int):
    smoothed = strip_lsb_image(cover_channel, n_bits)
    edge_map = _build_edge_map(smoothed)
    message_length = len(message_bits)

    stego = cover_channel.copy()
    tc = np.zeros_like(cover_channel, dtype=np.uint8)
    bit_index = 0
...
def embed_rgb(cover_path: str, secret_text: str, n_bits: int):
    # Citim imaginea color
    cover = cv2.imread(cover_path, cv2.IMREAD_COLOR)
    if cover is None:
        raise FileNotFoundError(f"Image not found at {cover_path}")
    cover = cv2.resize(cover, IMAGE_SIZE)
    b, g, r = cv2.split(cover)

    message_bits = text_to_bits(secret_text)
    total_message_length = len(message_bits)

    # Înglobăm secvențial în canalele B, G, R
    stego_b, tc_b, embedded_b = _embed_channel(b, message_bits, n_bits)
    stego_g, tc_g, embedded_g = _embed_channel(g, message_bits[embedded_b:], n_bits)
    stego_r, tc_r, embedded_r = _embed_channel(r, message_bits[embedded_b + embedded_g:], n_bits)

    total_embedded = embedded_b + embedded_g + embedded_r
    stego = cv2.merge([stego_b, stego_g, stego_r])
    tc_rgb = (tc_b, tc_g, tc_r)

    return stego, tc_rgb, total_embedded
...
def embed_all_images(cover_dir: str, stego_dir: str, secret_text: str, n_bits: int):
    ...
    for filename in filenames:
        ...
        print(f"  Embedding RGB into {filename} ...")
        stego, _, _ = embed_rgb(cover_path, secret_text, n_bits)
        cv2.imwrite(stego_path, stego)

        display_name = filename.split('.')[0].capitalize()
        # Salvăm RGB pt vizualizare matplotlib (matplotib folosește RGB, opencv folosește BGR)
        stego_disp = cv2.cvtColor(stego, cv2.COLOR_BGR2RGB)
        results.append((display_name, stego_disp))
...
if __name__ == "__main__":
    ...
    print("Running batch RGB embedding ...")
