...
from extraction import (_build_edge_map, _ring_coords, _local_pattern,
                         _bgr_to_ycocg, _ycocg_to_bgr, _filter_safe_positions)


def _extract_from_channel_corrected(stego, tc, edge_positions, edge_pattern, bit_index):
...
def extract_ycbcr_corrected(stego_image, tc_ycc, n_bits: int, total_message_length: int):
    ...
    for i in range(1, rows - 1, 3):
        for j in range(1, cols - 1, 3):
            ...
            bits = _extract_from_channel_corrected(stego_channels[target_idx], tc_channels[target_idx],
...
if __name__ == "__main__":
    ...
    text, restored = extract_ycbcr_corrected(stego, tc_ycc, N_BITS, msg_len)
